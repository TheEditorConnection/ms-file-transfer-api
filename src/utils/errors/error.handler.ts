...
export const handleError = async (
    ...
    try {
        ...
        if (error instanceof Error) {
            ...
            Logger.createTransactionLogger(transferId, `Error in transfer process for ID: ${transferId}`, error as Error);
        } else {
            // Handle non-Error object cases gracefully
            const unknownError = new Error(String(error));
            Logger.error(`Unknown error type in transfer ID: ${transferId}`, unknownError);
        }

        // Skip notification if the error is already a NotificationError
        if (error instanceof NotificationError) {
            Logger.info(`Skipping notification for transfer ID: ${transferId} due to a NotificationError.`);
            return;
        }
        ...
        try {
            ...
            Logger.info(`Notification sent successfully for transfer ID: ${transferId}`);
        } catch (notificationError) {
            Logger.createTransactionLogger(transferId, `Notification failure for transfer ID: ${transferId}`, notificationError as Error);
            Logger.error(`Failed to send notification for transfer ID: ${transferId}`, notificationError as Error);
        }
    } catch (unexpectedError) {
        // Catch any unforeseen errors within handleError itself
        Logger.error(`Unexpected error in handleError for transfer ID: ${transferId}`, unexpectedError as Error);
    }
};