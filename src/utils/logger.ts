...
const s3Client = isProduction ? new S3Client({ region: Config.get('LOG_S3_REGION') }) : null;
...
async function getExistingLogContentFromS3(filename: string): Promise<string> {
    ...
    try {
        ...
        // Log no encontrado; devolver vacío para comenzar uno nuevo
        return '';
    }
}
...
export const Logger = {
    ...
    createTransactionLogger(transferId: string, message: string, error?: Error): void {
        ...
        const transactionMessage = error ? `${message} - ${error.message}` : message;
