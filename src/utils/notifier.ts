...
export const notify = async (url: string, token: string, payload: IClientPayload): Promise<void> => {
    try {
        ...
        const notificationError = new NotificationError(url, payload, error as Error);
        Logger.error(`Transfer ID: ${payload.transferId}, Failed to send notification`, notificationError);
        Logger.createTransactionLogger(payload.transferId, `Failed to send notification for URL: ${url}`, notificationError);
    }
};