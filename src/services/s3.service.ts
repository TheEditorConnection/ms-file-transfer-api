...
export class S3Service {
  ...
  public async uploadFileStream(
  ...
  ): Promise<{ signedUrl: string; objectUrl: string; s3Key: string }> {
    const MAX_RETRIES = 3;
    let attempt = 0;
    ...
    while (attempt < MAX_RETRIES) {
      try {
        Logger.info(
          `Uploading file stream to S3: ${filePath}, attempt ${attempt + 1}, Google Drive File ID: ${googleDriveFileId}`,
        );
        ...
        upload.on("httpUploadProgress", (progress) => {
          if (progress.loaded) {
            ...
            Logger.info(
              `Upload progress for Google Drive File ID ${googleDriveFileId}: ${percentage.toFixed(2)}%`,
            );
          } else {
            Logger.warn(
              `Progress information not available for Google Drive File ID: ${googleDriveFileId}`,
            );
          }
        });
        ...
        await upload.done();
        Logger.info(
          `File stream uploaded successfully to S3 for Google Drive File ID: ${googleDriveFileId}, Path: ${filePath}`,
        );
        ...
        Logger.error(
          `Error uploading file stream to S3: ${filePath} for Google Drive File ID: ${googleDriveFileId}`,
          error,
        );
        ...
        Logger.info(
          `Retrying upload for file: ${filePath}, Google Drive File ID: ${googleDriveFileId}`,
        );
        attempt++;
      }
    }
  ...
  public async downloadFileAsStream(
    filePath: string,
