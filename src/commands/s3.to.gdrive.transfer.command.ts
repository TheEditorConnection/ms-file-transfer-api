...
export class S3ToGDriveTransferCommand {
    ...
    public async execute(): Promise<void> {
        ...
        Logger.createTransactionLogger(this.transferId, `Starting process to upload S3 file: ${this.s3FilePath} to Google Drive`);
        ...
        try {
            ...
            Logger.createTransactionLogger(this.transferId, `Final file name determined: ${finalFileName}`);
            ...
            Logger.createTransactionLogger(this.transferId, `File uploaded to Google Drive successfully, Drive File ID: ${driveFileId}`);
            ...
            Logger.logProcessingTime(this.transferId, duration, TransferDirection.S3_TO_GDRIVE);
            Logger.info(`Process completed at ${endTime.toISOString()}, total time: ${duration.toFixed(2)} seconds`);
            Logger.createTransactionLogger(this.transferId, `Process completed at ${endTime.toISOString()}, total time: ${duration.toFixed(2)} seconds`);
