...
export class GDriveToS3TransferCommand {
  ...
  public async execute(): Promise<void> {
    ...
    Logger.info(
      `Starting streaming upload process for Google Drive file ID: ${this.googleDriveFileId} at ${startTime.toISOString()}`,
    );
    Logger.createTransactionLogger(
      this.transferId,
      `Starting streaming upload process for Google Drive file ID: ${this.googleDriveFileId}`,
    );

    try {
      const fileName = await this.googleDriveService.getFileName(
        this.googleDriveFileId,
      );
      const fileSize = await this.googleDriveService.getFileSize(
        this.googleDriveFileId,
      );
      const filePath = `google_drive_upload/${this.projectId}/${this.deliveryItemId}/${this.googleDriveFileId}_${fileName}`;
      Logger.info(
        `Google Drive ID: ${this.googleDriveFileId}, File Name: ${fileName}, File Size: ${fileSize} bytes`,
      );
      Logger.createTransactionLogger(
        this.transferId,
        `Google Drive ID: ${this.googleDriveFileId}, File Name: ${fileName}, File Size: ${fileSize} bytes`,
      ...
      const driveStream = await this.googleDriveService.downloadFileAsStream(
        this.googleDriveFileId,
      );
      const { signedUrl, objectUrl, s3Key } =
      ...
      Logger.createTransactionLogger(
        this.transferId,
        `Google Drive file ID: ${this.googleDriveFileId} uploaded to S3 successfully`,
      );
      ...
      Logger.logProcessingTime(
        this.transferId,
        duration,
        TransferDirection.GDRIVE_TO_S3,
      );
      Logger.info(
        `Google Drive file ID: ${this.googleDriveFileId} uploaded successfully total time taken: ${duration.toFixed(2)} seconds`,
      );
      Logger.createTransactionLogger(
        this.transferId,
        `Google Drive file ID: ${this.googleDriveFileId} uploaded successfully total time taken: ${duration.toFixed(2)} seconds`,
